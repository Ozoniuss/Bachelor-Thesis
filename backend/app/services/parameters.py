...
def validate_training_parameters(params: TrainingParameters):
    ...
    """
    errors = ""
    if (params.seed > 256) or (params.seed < 1):
        errors += f"Invalid seed ({params.seed}): must be between 1 and 256\n"
    ...
    if params.epochs > 30:
        errors += (
            f"Invalid number of epochs ({params.epochs}): must be less or equal to 30\n"
        )
